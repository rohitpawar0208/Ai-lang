...
export function AliaChatInterfaceComponent() {
  ...
  const startConversation = (topicId: string, mode: 'chat' | 'voice') => {
    if (mode === 'chat') {
      ...
      if (topic) {
        router.push(`/user-page/talk-to-coach-communication/chat?topic=${topic.title}&category=${topic.category}&difficulty=${topic.difficulty}`)
      }
    } else {
      console.log(`Starting ${mode} conversation for topic: ${topicId}`)
    }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      <header className="bg-white p-4 flex items-center justify-between shadow-sm">
        ...
        <h1 className="text-xl font-semibold">ALIA AI Communication Coach</h1>
              ...
              {filteredTopics.map((topic) => (
                <Card key={topic.id} className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-gray-600 mb-4">Practice {topic.title.toLowerCase()} with ALIA</p>
                    <div className="grid grid-cols-2 gap-2">
